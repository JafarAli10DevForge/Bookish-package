...
export class AnalyticsApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to analyticsControllerGetAnalytics
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async analyticsControllerGetAnalyticsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<AnalyticsResponseDTO >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AnalyticsResponseDTO = ObjectSerializer.deserialize(
