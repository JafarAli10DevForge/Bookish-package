...
export class AuthApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async authControllerAddFeedback(feedbackPayloadDto: FeedbackPayloadDto, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
        ...
        if (feedbackPayloadDto === null || feedbackPayloadDto === undefined) {
            throw new RequiredError("AuthApi", "authControllerAddFeedback", "feedbackPayloadDto");
        }


        // Path Params
        const localVarPath = '/v1/auth/feedback';
        ...
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(feedbackPayloadDto, "FeedbackPayloadDto", ""),
            contentType
        );
...
export class AuthApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to authControllerAddFeedback
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async authControllerAddFeedbackWithHttpInfo(response: ResponseContext): Promise<HttpInfo<FeedbackCreateResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: FeedbackCreateResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "FeedbackCreateResponseDto", ""
            ) as FeedbackCreateResponseDto;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
